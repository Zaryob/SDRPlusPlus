...
void FrequencySelect::draw() {
    ...
    if (!gui::mainWindow.lockWaterfallControls) {
        ...
        if (isInArea(mousePos, digitTopMins[0], digitBottomMaxs[11])) {
            bool shortcutKey = io.ConfigMacOSXBehaviors ? (io.KeyMods == ImGuiKey_ModShift) : (io.KeyMods == ImGuiKey_ModCtrl);
            bool ctrlOnly = (io.KeyMods == ImGuiKey_ModCtrl);
            bool shiftOnly = (io.KeyMods == ImGuiKey_ModShift);
...
void FrequencySelect::setFrequency(int64_t freq) {
    flog::verbose("Frequency Select {0} Hz", freq);
